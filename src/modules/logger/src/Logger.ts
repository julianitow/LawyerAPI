...
export class Logger implements IModule {
  ...
  private static print(level: LEVEL, out: []): void {
    const now = new Date();
    const logFunc = level === LEVEL.ERROR ? process.stderr.write.bind(process.stderr) : process.stdout.write.bind(process.stdout);
    logFunc(
      `${Logger.colorMap.get(level)?.begin}[${level}]${Logger.colorMap.get(level)?.end}::${
